...
public class FirebaseAuthHandler : MonoBehaviour
{
    ...
    private void OnGmailLoginSuccess(string json)
    {
        ...
        FirebaseHandler.Instance.tempdata = new UserData
        {
        ...
        FirebaseHandler.Instance.firebaseDBHandlerInstance.uidText.text = FirebaseHandler.Instance.tempdata.uid;

        UIHandler.Instance.SwitchToHomePanel();

        string _data = Newtonsoft.Json.JsonConvert.SerializeObject(FirebaseHandler.Instance.tempdata);
        print("*********** " + _data);
