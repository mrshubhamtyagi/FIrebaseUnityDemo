mergeInto(LibraryManager.library, {
    ...
    UpdateJSON: function(path, value, objectName, onSuccess, onFailed) {
        var parsedPath = UTF8ToString(path);
        var parsedValue = UTF8ToString(value);
        var parsedObjectName = UTF8ToString(objectName);
        var parsedOnSuccess = UTF8ToString(onSuccess);
        ...
        try {
            firebase.database().ref(parsedPath).update(JSON.parse(parsedValue)).then(function(result) {
     ...
     UpdateKVJSON: function(path, key, value, objectName, onSuccess, onFailed) {
        var parsedPath = UTF8ToString(path);
        ...
        var parsedOnSuccess = UTF8ToString(onSuccess);
        var parsedOnFailed = UTF8ToString(onFailed);
        console.log("js-" + parsedKey);
        try {
            firebase.database().ref(parsedPath).update(
                { displayName: JSON.stringify(parsedValue)}
