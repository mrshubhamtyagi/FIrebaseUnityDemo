...
public class FirebaseDBHandler : MonoBehaviour
{
    ...
    public void UpdateJSON()
    {
        if (string.IsNullOrWhiteSpace(pathInputField.text))
        {
            ...
            {
                string _path = FirebaseHandler.Instance.tempdata.uid;
                print("PATH - " + _path);
                print("KeyValue - displayName | " + nameInputField.text);
                FirebaseDBLibrary.UpdateKVJSON(_path, "displayName", nameInputField.text, gameObject.name, "DisplayInfo", "DisplayErrorObject");
            }
            else DisplayErrorObject("Name is Empty");
        }
        else
        {
            if (!string.IsNullOrWhiteSpace(nameInputField.text))
            {
                string _path = pathInputField.text;
                print("PATH - " + _path);
                print("KeyValue - " + keyInputField.text + " | " + nameInputField.text);
                //FirebaseDBLibrary.UpdateJSON(_path, nameInputField.text, gameObject.name, "DisplayInfo", "DisplayErrorObject");
                FirebaseDBLibrary.UpdateKVJSON(_path, keyInputField.text, nameInputField.text, gameObject.name, "DisplayInfo", "DisplayErrorObject");
            }
            else DisplayErrorObject("Name is Empty");
        }
    }

    public class Key_Vaue
    {
        public string key;
        public string value;
    }
